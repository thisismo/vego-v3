 ...
 * drafted specification documents. In the rethought workflow the agent **self-heals** against this
 * report (rewriting files until it is clean) *before* streaming it to the IDE for the final
 * Finalize/Revise decision — so by the time the user sees it, `passed` should normally be true.
 */
@Serializable
